...
def load_marker_positions2(keys: list[str], data_dict: dict[str, list[float]])->npt.NDArray[np.float32]:
    data = []
    for k in keys:
        data.append(np.array(data_dict[k]))
    return np.array(data)

def plot_marker_positions(data_before: npt.NDArray[np.float32], data_after: npt.NDArray[np.float32]):
    fig = plt.figure()
    ax = fig.add_subplot(111, projection="3d")
    ax.scatter(data_before[:, 0], data_before[:, 1], data_before[:, 2], c="r", label="Before")
    ax.scatter(data_after[:, 0], data_after[:, 1], data_after[:, 2], c="b", label="After")
...
def main():
    ...
    displacement_field.fit(train_before, displacement[:, 0], displacement[:, 1], displacement[:, 2])
